...
- Claude Design 핸드오프 URL 이식 프롬프트가 들어오면 디자인 룬 직후의 슬라이드 12 상태를 만든다. 첫 화면의 시각 우선순위와 컴포넌트 질감은 반영하되, 데이터 분리, Supabase, Auth, API route, 배포, Tailwind 전면 전환, FCM은 이후 시련으로 남긴다.

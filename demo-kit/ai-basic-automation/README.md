...
> 30대 직장인 마케터 김민지의 5월 시나리오로 5단계를 따라가며 마지막에 HTML 한 페이지 "마케터 사령탑" 대시보드를 만드는 데모 팩.
>
...
└── samples/
    ...
    ├── step5-dashboard-starter.html                (실습 5 시작점 — 마케터 사령탑 6 위젯)
...
- 욕구: "캠페인·고객·생활이 한 화면에 모이는 **마케터 사령탑**"
...
| 5 대시보드 만들기 ⭐ | **업무+생활 통합** | 마케터 사령탑 — 캠페인·SNS·KPI + 고정지출·루틴·캘린더 |
...
### 5-2. 프롬프트 5 — 마케터 사령탑 (6 위젯)

```text
30대 직장인 마케터의 5월 사령탑 대시보드를 HTML 한 페이지로 만들어줘.
오른쪽 화면(Claude) 또는 미리보기 화면(ChatGPT)에서 바로 보이게.

상단 헤더: "김민지의 5월 사령탑" + 부제 "캠페인 · KPI · 생활 통합"
...
화면 생성이 어렵다면 본인 마케터 사령탑 구성 요소 5개 + 우선순위 손글씨로 작성. 완료 인정.
